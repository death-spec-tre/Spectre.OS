...
function resizeRect(start: WindowInstance, dir: Dir, dx: number, dy: number, minW: number, minH: number) {
  let { x, y, w, h } = start;
...
export default function Window({ win, children }: { win: WindowInstance; children: ReactNode }) {
  ...
  const exiting = os.exiting[win.id];
  ...
  return (
    <>
      ...
      <div
        className={cn(
          "os-skin absolute flex flex-col overflow-hidden border bg-[#07080b]/95 text-ink backdrop-blur-[2px]",
          exiting === "closing" ? "win-out" : exiting === "minimizing" ? "win-minimize" : "win-in",
      ...
      >
        ...
        >
          ...
          <div className="ml-auto flex items-center gap-3 sm:gap-5" data-no-drag>
            <CtrlBtn label="Minimize" mobile={isMobile} onClick={() => os.requestMinimize(win.id)}>
              ─
            </CtrlBtn>
            <CtrlBtn label="Maximize" mobile={isMobile} onClick={() => os.toggleMaximize(win.id)}>
              □
            </CtrlBtn>
            <CtrlBtn label="Close" danger mobile={isMobile} onClick={() => os.requestClose(win.id)}>
              ✕
            </CtrlBtn>
          </div>
        </div>

        <div className="os-scroll relative min-h-0 flex-1 overflow-auto bg-[#050609]/85 backdrop-blur-sm">{children}</div>
