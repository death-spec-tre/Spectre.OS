...
const BOOT_LOG = [
  "SPECTRE BIOS v1.3 — (c) Death Spectre Systems",
...
export default function BootScreen({ onDone }: { onDone: () => void }) {
  ...
  return (
    <div
      onPointerDown={skip}
      className={`scanlines os-skin fixed inset-0 z-[200000] flex flex-col items-center justify-center overflow-hidden bg-void px-4 ${stage === "done" ? "opacity-0 transition-opacity duration-500" : ""}`}
    >
      {stage === "logs" && (
      ...
      {stage !== "logs" && (
        <div className="fade-in flex w-full max-w-md flex-col items-center text-center">
          ...
          <div className="mt-1 font-mono text-[11px] tracking-[0.25em] text-faint">v1.3</div>

          <div className="mt-8 w-full font-mono text-[12px] text-muted">Initializing...</div>
          <div className="mt-2 w-full select-none font-mono text-[15px] tracking-tight text-accent">[{bar}]</div>
          ...
          {stage === "done" && (
            <div className="fade-in mt-8 space-y-1 font-mono">
              <div className="text-[13px] font-semibold tracking-[0.2em] text-accent">▣ USER DETECTED</div>
      ...
      <div className="absolute bottom-5 font-mono text-[10px] tracking-widest text-dim">press any key to skip</div>
    </div>
  );
}
