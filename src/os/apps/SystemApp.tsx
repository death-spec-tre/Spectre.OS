      ...
      <div className="os-scroll min-h-0 flex-1 overflow-auto p-5">
        ...
        <div className="mt-7 flex flex-wrap items-end justify-between gap-4 border-t border-edge pt-4">
          <div className="font-mono text-[11px] text-faint">
            <div>
              session uptime <span className="text-accent">{mm}:{ss}</span>
            </div>
            <div>processes: {os.windows.length + 7} · threads: ∞</div>
          </div>
          <button
            type="button"
            onClick={() => os.notify("SYSTEM", "Have you tried turning it off and on?", "warn")}
            className="no-tap border border-edge2 bg-panel px-2.5 py-1 text-[11px] text-muted hover:border-edge3 hover:text-ink"
          >
            ⟳ restart human
          </button>
        </div>
