  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="shrink-0 border-b border-edge bg-panel/60 px-3 py-2">
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5">
          ...
          <span className="micro text-faint">
            REPOSITORIES <span className="text-ink">{totalRepos}</span> DETECTED
          </span>
          <span className="micro text-faint">
            INDEXED <span className="text-ink">{projects.length}</span>
          </span>
      ...
      <div className="os-scroll min-h-0 flex-1 overflow-auto p-3">
        {visible.length === 0 ? (
          <div className="micro p-4 text-faint">
        ...
        <div className="mt-4 border-t border-dashed border-edge pt-2 micro text-faint">
          ▸ auto-indexed from GitHub topic <span className="text-muted">spectre-portfolio</span> — add it to a repo to publish it here
        </div>
      </div>
    </div>
  );
    ...
    <div className="flex flex-col border border-edge bg-panel p-3 transition-colors hover:border-edge3 hover:bg-panel2">
      ...
      <div className="flex items-start justify-between gap-2">
        <span className="flex min-w-0 items-center gap-1.5">
          {p.featured && (
            <span className="shrink-0 text-accent" title="Featured project">★</span>
          )}
